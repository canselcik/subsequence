...
public class Callbacks extends Controller {
    ...
    private static ObjectNode processDeposit(Transaction.Deposit d, String txHash, BitcoindInterface bi){
        ...
        if(presence.getValue() == TransactionDB.TxDatabasePresence.NOT_PRESENT.getValue()){
            ...
            if(confirmed && !TransactionDB.addTxInputsToDB(rt)){
                result.put("error", "Failed to add tx inputs to DB");
                return result;
            }
    ...
    public static Result txNotify(String payload) {
        ...
        for(Transaction.Deposit d : deposits){
            ObjectNode result = processDeposit(d, payload, bi);
            results.add(result);
        }

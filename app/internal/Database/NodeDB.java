...
public class NodeDB {
    public static class BitcoindNodeInfo {
        ...
        public final String externalIP, humanReadableName, rpcUsername, rpcPassword;
        public final BitcoindClientFactory factory;
        public BitcoindNodeInfo(int id, String humanReadableName, String externalIP, int bitcoindPort, int httpPort,
                                String rpcUsername, String rpcPassword, BitcoindClientFactory factory, int accountCount){
            this.id = id;
    ...
    public static List<BitcoindNodeInfo> getBitcoindNodes(){
        ...
        try {
            ...
            while(rs.next()){
                ...
                BitcoindClientFactory bcf = new BitcoindClientFactory(new URL("http://" + externalIP + ":" + bitcoindPort), rpcUsername, rpcPassword);
                clusters.add( new BitcoindNodeInfo(id, humanReadableName, externalIP, bitcoindPort, httpPort,
    ...
    public static boolean registerLocalNode(){
        ...
        try {
            PreparedStatement ps = conn.prepareStatement("INSERT INTO bitcoind_nodes(human_readable_name, external_ip, " +
                    "http_port, bitcoind_port, rpc_username, rpc_password, account_count) VALUES (?, ?, ?, ?, ?, ?, ?)");
            ps.setString(1, humanReadableName);
            ps.setString(2, externalIP);
            ps.setInt(3, httpPort);
            ps.setInt(4, bitcoindPort);
            ps.setString(5, rpcUsername);
            ps.setString(6, rpcPassword);
            ps.setInt(7, 0);
            int affectedRows = ps.executeUpdate();
            conn.close();
    ...
    public static BitcoindNodeInfo getNodeInformationByName(String name){
        ...
        try {
            ...
            String externalIP = rs.getString("external_ip");
            BitcoindClientFactory bcf = new BitcoindClientFactory(new URL("http://" + externalIP + ":" + bitcoindPort), rpcUsername, rpcPassword);
            return new BitcoindNodeInfo(id, humanReadableName, externalIP, bitcoindPort, httpPort, rpcUsername, rpcPassword, bcf, clusterSize);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
